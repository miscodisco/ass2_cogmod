...
#0 = right, 1 = left
#Right and win = 1, lose and left = 1, right and lose = -1, left and win = -1
...
subdata <- list(n = 119, c = sim_df$self, outcome_hand = sim_df$outcome_hand) # why is one row missing?
...
ggplot(draws_df) +
  geom_histogram(aes(prior_preds_minus), color = "lightblue", fill = "blue", alpha = 0.3, bins = 90) +
  geom_histogram(aes(posterior_preds_minus), color = "darkblue", fill = "blue", alpha = 0.3, bins = 90) +
...
```{r}
file <- file.path("prior_k_attempt.stan")
mod <- cmdstan_model(file, 
                     cpp_options = list(stan_threads = TRUE),
                     stanc_options = list("O1"))
...
```{r}
data <- list(n = 1199, c = sim_df$self, outcome_hand = sim_df$outcome_hand,  prior_mean_bias = priors$prior_mean_bias, prior_sd_bias = priors$prior_sd_bias, prior_mean_follow_bias = priors$prior_mean_follow_bias, prior_sd_follow_bias = priors$prior_sd_follow_bias) # why is one row missing?
