...
```{r}
prior_mean_bias <- seq(-1, 1, .25)
prior_sd_bias <- seq(0.1, 1.1, 0.2)
prior_mean_follow_bias <- seq(-3, 3, 0.5)
prior_sd_follow_bias <- seq(0.1, 1.1, 0.2)
...
```{r}
file <- file.path("Prior_sens.stan")
...
sim_d_and_fit <- function(prior_mean_bias, prior_sd_bias, prior_mean_follow_bias, prior_sd_follow_bias) {
    ...
    draws_df <- as_draws_df(samples$draws()) 
      temp <- tibble(bias_prior = draws_df$bias_prior, 
                     ...
                     prior_mean_follow_bias = prior_mean_follow_bias,
                     prior_sd_follow_bias = prior_sd_follow_bias)
...
ggplot(recovery_df, aes(prior_mean_follow_bias, follow_bias_posterior)) +
  ...
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(.~prior_sd_follow_bias) +
  theme_classic()
...
### MEMORY SIMULATION AND PRIOR SENSITIVY ANALYSIS BEGIN HERE
```{r}
# set up
ntrials <- 120
nagents <- 10
...
data <- list(n = 1200, h = d1$self, other = d1$other, prior_mean_bias = 0,prior_sd_bias = 1,  prior_mean_beta = 0, 
             prior_sd_beta = 1,prior_mean_conf = 0, prior_sd_conf = 0.3 )
file <- file.path("memo.stan")
mod <- cmdstan_model(file, 
                     cpp_options = list(stan_threads = TRUE),
                     stanc_options = list("O1"))
...
```{r}
#Priors
prior_mean_bias <- seq(-1, 1, 1)
prior_sd_bias <- seq(0.1, 1.1, .5)
prior_mean_beta <- seq(-1, 1, 1)
prior_sd_beta <- seq(0.1, 1.1, .5)
prior_mean_conf <- seq(-1, 1, 1)
prior_sd_conf <- seq(0.1, 1.1, .5)
priors <-  expand.grid(prior_mean_bias, prior_sd_bias, prior_mean_beta, prior_sd_beta, prior_mean_conf, prior_sd_conf)
priors <- tibble(prior_mean_bias = priors$Var1, prior_sd_bias = priors$Var2, prior_mean_beta = priors$Var3, prior_sd_beta = priors$Var4, prior_mean_conf = priors$Var5, prior_sd_conf = priors$Var6)
...
```{r}
file <- file.path("mem_prior_sens.stan")
...
sim_d_and_fit <- function(prior_mean_bias, prior_sd_bias, prior_mean_beta, prior_sd_beta, prior_mean_conf, prior_sd_conf) {

    data <- list(n = 1200, h = d1$self, memory = d1$memory, confidence = d1$confidence, prior_mean_bias = prior_mean_bias, prior_sd_bias = prior_sd_bias, prior_mean_beta = prior_mean_beta, prior_sd_beta = prior_sd_beta, prior_mean_conf = prior_mean_conf, prior_sd_conf = prior_sd_conf)
    ...
    draws_df <- as_draws_df(samples$draws()) 
      temp <- tibble(bias_prior = draws_df$bias_prior, 
                     ...
                     conf_prior = draws_df$conf_prior,
                     conf_posterior = draws_df$conf_posterior,
                     prior_preds = draws_df$prior_preds, 
                     posterior_preds = draws_df$posterior_preds, 
                     prior_mean_bias = prior_mean_bias,
                     prior_sd_bias = prior_sd_bias,
                     ...
                     prior_mean_conf = prior_mean_conf,
                     prior_sd_conf = prior_sd_conf)
    
    return(temp)
  
}

recovery_df1 <- future_pmap_dfr(priors[1:50,], sim_d_and_fit, .options = furrr_options(seed = TRUE))

```

```{r}

ggplot(recovery_df1, aes(prior_mean_bias, bias_posterior)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0.6, color = "red") +
  geom_smooth(method = lm, se = FALSE)+
  facet_wrap(.~prior_sd_bias) +
...
ggplot(recovery_df1, aes(prior_mean_beta, beta_posterior)) +
  geom_point(alpha = 0.1) +
...
ggplot(recovery_df1, aes(prior_mean_conf, conf_posterior)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0.1, color = "red") +
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(.~prior_sd_conf) +
  theme_classic()
